...
- **Сбор данных:** картинка, название товара, номер заказа, дата, цена, статус доставки, информация об оплате/количестве
- **Автоматическая фильтрация** отменённых заказов (не попадают в таблицу)
...
- **Фильтры** по диапазону дат и цен
- **Просмотр изображений** товаров в полном размере (модальное окно)
- **Экспорт** в автономный HTML-файл (работает без интернета)
- **Настройка лимита** собираемых заказов (25 / 50 / 100 / 200 / 500)
...
| `result.html` | Разметка страницы с таблицей результатов |
| `result.css` | Стили страницы результатов |
